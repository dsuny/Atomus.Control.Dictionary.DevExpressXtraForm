...
{
    public partial class DevExpressXtraForm : XtraForm, IDictionaryForm
    {
        IDictionary IDictionaryForm.Dictionary { get; set; }

        #region Init
        public DevExpressXtraForm()
        {
            InitializeComponent();
        }

        private void GridInit(GridControl gridControl)
        {
            IDataGridAgent gridAgent;

            gridAgent = this.DataGridAgent(gridControl);
            //gridAgent = new DevExpressGridControlAgent();
            //gridAgent.GridControl = gridControl;
            gridAgent.Init(EditAble.False, AddRows.False, DeleteRows.False, ResizeRows.False, AutoSizeColumns.False, AutoSizeRows.False, ColumnsHeadersVisible.True, EnableMenu.True, MultiSelect.False, Grid.Alignment.MiddleCenter, -1, 1, -1, RowHeadersVisible.False, Selection.FullRowSelect);

            ((GridView)gridControl.MainView).OptionsView.ShowGroupPanel = false;

            ((GridView)gridControl.MainView).OptionsFilter.AllowFilterEditor = false;
            ((GridView)gridControl.MainView).OptionsFilter.AllowMRUFilterList = false;
        }
        #endregion
        ...
        #region Spread
        private void SetGrid(GridControl dataGridView, DataView dataView, IDictionary dictionary)
        {
            IDataGridAgent gridAgent;
            Grid.Alignment textAlign;
            string tmp;
            string[] caption;
            string[] split;
            int width;
            ...
            try
            {
                ...
                //gridAgent = new DevExpressGridControlAgent();
                //gridAgent.GridControl = dataGridView;
                gridAgent.Clear();
                ...
                foreach (DataColumn dataColumn in dataView.Table.Columns)
                {
                    if (dataColumn.DataType.IsNumeric())//숫자일 경우에 오른쪽 정렬
                        textAlign = Grid.Alignment.MiddleRight;
                    else
                        textAlign = Grid.Alignment.MiddleLeft;
        ...
        #region Event
        private void DevExpressXtraForm_Load(object sender, EventArgs e)
        {
            this.Opacity = 0;//투명하게
            ...
            this.GridInit(this.gridControl1);
        }
        ...
        private void DataGridView1_CellDoubleClick(object sender, EventArgs e)
        {
            ...
            dataGridView = (GridView)((GridControl)sender).MainView;

            if (dataGridView.RowCount < 1)
                return;

            if (dataGridView.GetDataRow(dataGridView.GetSelectedRows()[0]) != null)
        ...
        private void DataGridView1_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            try
            {
                ...
                if (e.KeyCode == Keys.Up)
                    if (dataGridView.GetSelectedRows().Length == 0)
                ...
                if (e.KeyCode == Keys.Enter)
                {
                    ...
                    if (dataGridView.GetSelectedRows().Length == 1)
                    {
                        dataRowView = (DataRow)dataGridView.GetDataRow(dataGridView.GetSelectedRows()[0]);//선택행 가져오기

                        if (dataRowView != null)
                            afterEventArgs.DataRow = dataRowView;
                        else
        ...
        private void Text_Search_TextChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (this.Visible)//Lov 폼이 활성화 상태에서
                {
                    ...
                    if (dictionary.CurrentControl is TextEdit || dictionary.CurrentControl is ButtonEdit)
                        dictionary.CurrentControl.Text = control.Text;//검색 문자열 반영

                    dataView = (GridView)this.gridControl1.MainView;
                    ...
                    if (!text.Equals(""))//쿼리로 검색된 내용에서 RowFilter
                    {
                        ...
                        index = -1;
                        foreach (GridColumn _DataColumn in ((GridView)this.gridControl1.MainView).Columns)
                        {
                            if (_DataColumn.Visible && _DataColumn.Width > 0) //보이는 컬럼만 검색
                            {
                                ...
                                {
                                    if (_DataColumn.AppearanceCell.TextOptions.HAlignment == DevExpress.Utils.HorzAlignment.Far && _DataColumn.AppearanceCell.TextOptions.VAlignment == DevExpress.Utils.VertAlignment.Center)
                                        if (text.ToTryDecimal(out decimalTmp))
                                            if (beforeEventArgs.StartsWith)
                                                stringBuilder.AppendFormat("OR Convert([{0}], 'System.String') LIKE '{1}%' ", _DataColumn.FieldName, text);
                                            //_Tmp += "OR Convert([" + _DataColumn.DataPropertyName + "], 'System.String') LIKE '" + _Text + "%' ";
                                            else
                                                stringBuilder.AppendFormat("OR Convert([{0}], 'System.String') LIKE '%{1}%' ", _DataColumn.FieldName, text);
                                        //_Tmp += "OR Convert([" + _DataColumn.DataPropertyName + "], 'System.String') LIKE '%" + _Text + "%' ";
                                        else if (beforeEventArgs.StartsWith)
                                            stringBuilder.AppendFormat("OR [{0}] LIKE '{1}%' ", _DataColumn.FieldName, text);
                    ...
                    try
                    {
                        if (stringBuilder.ToString().StartsWith("OR "))
                            dataView.ActiveFilterString = stringBuilder.ToString(3, stringBuilder.Length - 3);
                        else
                            dataView.ActiveFilterString = stringBuilder.ToString();
                    }
                    catch (Exception exception)
                    {
                        dataView.ActiveFilterString = "";
                        DiagnosticsTool.MyTrace(exception);
                    }

                    if (dataView.DataSource != null)
                        this.Text = string.Format("{0} - {1} ({2}/{3})", ((DataView)dataView.DataSource).Table.TableName, this.Name, dataView.RowCount, ((DataView)dataView.DataSource).Table.Rows.Count);
                }
            }
            catch (Exception exception)
            {
            ...
            if (e.KeyCode == Keys.Enter)
                try
                {
                    ...
                    if (await task)
                    {
                        ...
                        this.SetGrid(this.gridControl1, afterEventArgs.DataTable.DefaultView, ((IDictionaryForm)this).Dictionary);
                        SetForm(this, this.gridControl1);
        ...
        private void CheckBox_SearchAll_CheckedChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                gridAgent = this.DataGridAgent(this.gridControl1);
                //_GridAgent.RemoveColumnFiter(this.txt_Search);
        ...
        #region "ETC"
        private void SetForm(XtraForm form, GridControl dataGridView)
        {
            int defaultWidth;
            int defaultHeight;
            int colWidth;
            ...
            try
            {
                ...
                GridViewInfo vi = (GridViewInfo)((GridView)dataGridView.MainView).GetViewInfo();
                DevExpress.XtraGrid.Scrolling.ScrollInfo scrollInfo = new DevExpress.XtraGrid.Scrolling.ScrollInfo(dataGridView.MainView);

                if (((GridView)dataGridView.MainView).OptionsView.ShowColumnHeaders) //최소 높이(검색박스 + 컬럼헤더 높이)
                    defaultHeight = 72 + vi.ColumnRowHeight;//((GridView)dataGridView.MainView).ColumnPanelRowHeight;
                else
                    defaultHeight = 72;

                rowHeight = vi.ColumnRowHeight;// ((GridView)dataGridView.MainView).RowHeight; //한 행의 높이
                colWidth = 0; //전체 컬럼의 폭
                scrollWidth = scrollInfo.VScroll.Width; //스크롤 넓이

                //DataGridView 폭 가져오기
                foreach (GridColumn _DataGridViewColumn in ((GridView)dataGridView.MainView).Columns)
                    if (_DataGridViewColumn.Visible)
                        colWidth += _DataGridViewColumn.Width + 2;
                ...
                {
                    switch (dataGridView.MainView.RowCount)
                    {
                        case 0:
                        ...
                        default:
                            ...
                            width = colWidth + defaultWidth;
                            height = defaultHeight + (rowHeight * dataGridView.MainView.RowCount);
